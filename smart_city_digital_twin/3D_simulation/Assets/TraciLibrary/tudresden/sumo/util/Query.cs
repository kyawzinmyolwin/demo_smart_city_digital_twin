...
namespace TraciConnector.Tudresden.Sumo.Util
{
    public abstract class Query
    {
        ...
        {
            // One NetworkStream per socket: two wrappers fight over the same TCP buffer on Mono/Unity
            // and commonly break TraCI right after a separate handshake stream on the same socket.
            NetworkStream ns = new NetworkStream(sock, ownsSocket: false);
            outStream = ns;
            inStream = ns;
        }
